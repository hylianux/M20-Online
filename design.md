...
It will run on NodeJS, utilizing a nosql database design and websockets.  The nosql database will be stored locally to the server rather than to a full database system like MongoDB.  This is because M20 Online isn't meant to be that cumbersome, and by locally storing everything, it decreases the dependencies for anyone wishing to use it.
